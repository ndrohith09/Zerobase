...
#*main
def main():
    cursor.execute('CREATE TABLE IF NOT EXISTS Department (id SERIAL PRIMARY KEY, id INT, name VARCHAR, manager_id INT)') 
    conn.commit()
...
class Query:
    ...
    def all_department(self) -> typing.List[Department]:
        ...
        department = []
        for i in lst:
            department.append(Department(id=i[0], id=i[1], name=i[2], manager_id=i[3]))
        return department
    ...
    def get_department(self, id: str) -> Department:
        ...
        lst = cursor.fetchone()
        return Department(id=lst[0], id=lst[1], name=lst[2], manager_id=lst[3])
...
class Mutation:
    #*graphmutation
    @strawberry.mutation
    def create_department(self, id: str, name: str, manager_id: str) -> Department:
        cursor.execute("INSERT INTO Department (id, name, manager_id) VALUES (%s, %s, %s) RETURNING id", (id, name, manager_id))
        conn.commit()
        department_id = cursor.fetchone()[0]
        return Department(id=department_id,id=id, name=name, manager_id=manager_id)
    
    @strawberry.mutation
    def update_department(self, id: str, id: str, name: str, manager_id: str) -> Department:
        
        cursor.execute("UPDATE Department SET id=%s, name=%s, manager_id=%s WHERE id = %s", (id, name, manager_id, id))
        conn.commit()
        return Department(id=id,id=id, name=name, manager_id=manager_id)
    ...
    def delete_department(self, id: str) -> Department:
        ...
        lst = cursor.fetchone()
        if lst is None:
            return Department(id='No Data Found',id='No Data Found', name='No Data Found', manager_id='No Data Found')
